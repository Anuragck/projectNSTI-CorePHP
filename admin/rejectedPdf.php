<?php
 ...
 //pdf generating code
    ...
    address,10thCertificateNo,10thMarkPercentage,12thCertificateNo,12thMarkPercentage FROM tbl_rejected");
    $header = $conn->Query("SELECT UCASE(`COLUMN_NAME`) 
    FROM `INFORMATION_SCHEMA`.`COLUMNS` 
    WHERE `TABLE_SCHEMA`='nsti_calicut' 
    AND `TABLE_NAME`='tbl_rejected'
    ...
    // Title
    $pdf->Cell(630,20,'NSTI Calicut - Rejected Students Details',1,0,'C');
    // Line break
    $pdf->Ln(20);
